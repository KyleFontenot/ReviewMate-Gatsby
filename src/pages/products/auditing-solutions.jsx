...
const Index = () => {
      ...
      allMarkdownRemark(
        filter: {
          frontmatter: {
            ...
            category: { eq: "Auditing Solutions" }
          }
        }
      ) {
        edges {
  ...
  return (
    <Layout slug={slug} pathName={`/products/${santizeSlug(slug)}`}>
      <h1>{slug}</h1>
      <Hero
        bgimgUniqueName="working-environment-and-discussion.jpg"
        maxHeight="30rem"
      ...
      >
        ...
        >
          <p>
            In order to manage the medical coding audit process, ReviewMate
            <span class="reg">&reg;</span> offers several options for data
            integration and mining. Data can seamlessly be integrated into the
            software a variety of ways. With FTP and API options, ReviewMate
            <span class="reg">&reg;</span> will pair easily with coding and
            abstracting systems as well as electronic health records. Wherever
            coded data resides, ReviewMate<span class="reg">&reg;</span> can
            consume the data to make the coding audit process faster and easier.
          </p>
        </div>
      </Hero>
          ...
          return (
            <CascadeBlockPlain key={item.node.id}>
              ...
              <div className="column cascadeBlock__text">
                ...
                <Link
                  to={`/products/auditing-solutions/${santizeSlug(
                    item.node.frontmatter.title
                  )}/`}
                  style={{
                    display: "block",
